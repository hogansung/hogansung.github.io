...
# hack: reverse left and right
PRED_TARGETS = ['over speed', 'too close', 'lane switch to right', 'lane switch to left']
...
class CarCanvas(FigureCanvas):
    def __init__(self, parent=None, dpi=300):
        self.xRange = (-100, 100)
        self.boundaries = (-100, 100, 0.7, -13)
    ...
    def drawBackground(self, xMin, xMax):
        ...
        xMin=1000
        xMax=10000
        self.lines[0] = plt.Rectangle([xMin-USER_CAR_LENGTH+self.xRange[0], self.lineY[0]-ROAD_LINE_WIDTH],
                                      xMax+self.xRange[1]-(xMin-USER_CAR_LENGTH+self.xRange[0]), ROAD_LINE_WIDTH, 
                                      ec='b', fc='b')
        self.lines[3] = plt.Rectangle([xMin-USER_CAR_LENGTH+self.xRange[0], self.lineY[3]-ROAD_LINE_WIDTH],
                                      xMax+self.xRange[1]-(xMin-USER_CAR_LENGTH+self.xRange[0]), ROAD_LINE_WIDTH, 
                                      ec='b', fc='b')
    ...
    def updateCars(self, c_pos, sc_infoList):
        self.car.set_xy((c_pos[0]-USER_CAR_LENGTH, c_pos[1]-USER_CAR_WIDTH/2+ROAD_LINE_WIDTH/2))
        ...
        for x, y, h, w in sc_infoList:
            if self.xRange[0] <= x-c_pos[0] and x-c_pos[0] <= self.xRange[1]:
                rect = plt.Rectangle((x-h, y-w/2+ROAD_LINE_WIDTH/2), h, w, fc='k')
        ...
        while (self.lines[1][-1].get_xy()[0] < c_pos[0]-USER_CAR_LENGTH/2+self.xRange[1]):
            ...
            self.lines[1].append(plt.Rectangle([lastPos[0]+ROAD_LINE_BLOCK, self.lineY[1]+ROAD_LINE_WIDTH/2], 
                                 ROAD_LINE_FILLED, ROAD_LINE_WIDTH, ec='b', fc='b'))
            self.axes.add_patch(self.lines[1][-1])
            self.lines[2].append(plt.Rectangle([lastPos[0]+ROAD_LINE_BLOCK, self.lineY[2]+ROAD_LINE_WIDTH/2], 
                                 ROAD_LINE_FILLED, ROAD_LINE_WIDTH, ec='b', fc='b'))
            self.axes.add_patch(self.lines[2][-1])
        
        while (self.lines[1][0].get_xy()[0] > c_pos[0]-USER_CAR_LENGTH/2+self.xRange[0]-ROAD_LINE_BLOCK):
            lastPos = self.lines[1][0].get_xy()
            self.lines[1].appendleft(plt.Rectangle([lastPos[0]-ROAD_LINE_BLOCK, self.lineY[1]+ROAD_LINE_WIDTH/2],
                                     ROAD_LINE_FILLED, ROAD_LINE_WIDTH, ec='b', fc='b'))
            self.axes.add_patch(self.lines[1][0])
            self.lines[2].appendleft(plt.Rectangle([lastPos[0]-ROAD_LINE_BLOCK, self.lineY[2]+ROAD_LINE_WIDTH/2],
                                     ROAD_LINE_FILLED, ROAD_LINE_WIDTH, ec='b', fc='b'))
...
#        self.setStyleSheet('background-color: darkgray')
    ...
    def addStatCanvas(self, layout):
        ...
        self.laneCanvas.drawBackground(self.data['Current Lane'], self.data['Time'], 'Lane (#)', 'Current Lane', [2.5,-0.6], [range(3), ['lhs', 'mhs', 'rhs']], labelbottom='on', margin=[0.23, 0.06])
