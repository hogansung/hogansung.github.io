...
class User:
    ...
    def addData(self, data, collision, timestamp, mode):
        ...
        for name in ['Dist to Front', 'Dist to Back', 'Dist to Left-Front', 'Dist to Left-Back', 'Dist to Right-Front', 'Dist to Right-Back']:
            new_data[name] = new_data[name].apply(lambda x: x <= 200 and x >= 0 else 200)
    ...
    def st_statistics(self, f_window, l_window, th=0.4, limit=200):
        ...
        close['Dist to Front'].where((close['Dist to Right-Front']<limit) & (close['Dist to Front']>0), other=limit, inplace = True)
        close['Dist to Left-Front'].where((close['Dist to Left-Front']<limit) & (close['Dist to Left-Front']>0), other=limit, inplace = True)
        close['Dist to Right-Front'].where((close['Dist to Right-Front']<limit) & (close['Dist to Right-Front']>0), other=limit, inplace = True)
        ...
        data['st_d_b_ave'] = self.weightedAverage(close['Dist to Back'],close['Duration'])
        data['st_d_lb_ave'] = self.weightedAverage(close['Dist to Left-Back'],close['Duration'])
        data['st_d_rb_ave'] = self.weightedAverage(close['Dist to Right-Back'],close['Duration'])
...
#        data['st_nc'] = int(l_window['Collisions'].sum() > 0)
        ...
        # over speed features
        ls_os = l_window[l_window['Velocity'] > 90.0/3.6]
...
lt = 150
gral.lt_setRangedStats(0, lt)
gral.lt_stats.to_csv('../fts/lt_stats.csv', index=False)

#for st_f in [30, 20, 10]:
#    for st_l in [5, 10]:
for st_f in [30]:
    for st_l in [5]:
        for i in range(lt, 300 - st_f, st_l):
            gral.st_setRangedStats(i, st_f, st_l)
        gral.st_stats.to_csv('../fts/st_stats' + '_' + str(st_f) + '_' + str(st_l) + '.csv', index=False)

#mult_win = gral.multipleWindows([(300,1),(150,50),(120,30),(100,25),(60,20)])
#mult_win.to_csv('../../../multiple_windows.csv', index=False)
